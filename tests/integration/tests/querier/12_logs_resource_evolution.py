...
def _test_logs_resource_evolution(
    ...
    """
    evolution_time = get_resource_evolution_time(o11y, "logs")
    ...
    before_series = _query_grouped_log_series(o11y, token, before_2 - timedelta(minutes=1), before_1 + timedelta(minutes=1))
    assert_grouped_series(
        before_series,
    ...
    after_series = _query_grouped_log_series(o11y, token, after_1 - timedelta(minutes=1), after_2 + timedelta(minutes=1))
    assert_grouped_series(
        after_series,
        expected_values_by_group={
            "svc-after-1": {
    ...
    spanning_series = _query_grouped_log_series(o11y, token, before_2, after_2 + timedelta(minutes=1))
    assert_grouped_series(
        spanning_series,
        expected_values_by_group={
            "svc-before-2": {
...
def test_logs_resource_evolution(
    ...
    _test_logs_resource_evolution(o11y, token, insert_logs)
