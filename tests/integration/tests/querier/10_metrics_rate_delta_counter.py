...
def test_rate_with_steady_values_and_reset(
    ...
    assert result_values[30]["value"] == 0.0333  # reset happens and [30] is for 31st minute. 2/60 cuz delta divides by step interval
    assert result_values[31]["value"] == 0.133  # i.e 8/60 i.e 31st to 32nd minute changes
...
def test_rate_group_by_endpoint(
    ...
    assert len(health_values) == 60, f"Expected 60 values for /health, got {len(health_values)}"
    count_steady_health = sum(1 for v in health_values if v["value"] == 0.167)
    assert count_steady_health == 60, f"Expected == 60 steady rate values (0.167) for /health, got {count_steady_health}"
    ...
    assert len(products_values) == 51, f"Expected 51 values for /products, got {len(products_values)}"
    count_steady_products = sum(1 for v in products_values if v["value"] == 0.333)

    assert count_steady_products == 51, f"Expected 51 steady rate values (0.333) for /products, got {count_steady_products}"
    ...
    assert len(checkout_values) == 61, f"Expected 61 values for /checkout, got {len(checkout_values)}"
    count_steady_checkout = sum(1 for v in checkout_values if v["value"] == 0.0167)
    assert count_steady_checkout == 56, f"Expected 56 steady rate values (0.0167) for /checkout, got {count_steady_checkout}"
    ...
    assert count_spike_checkout == 5, f"Expected 5 spike rate values (0.833) for /checkout, got {count_spike_checkout}"
    ...
    assert len(orders_values) == 60, f"Expected 59 values for /orders, got {len(orders_values)}"
    count_steady_orders = sum(1 for v in orders_values if v["value"] == 0.0833)
    assert count_steady_orders == 58, f"Expected 58 steady rate values (0.0833) for /orders, got {count_steady_orders}"
    ...
    assert len(high_rate_orders) == 1, f"Expected one high rate value after counter reset, got {non_standard_orders}"
    ...
    assert len(users_values) == 56, f"Expected 56 values for /users, got {len(users_values)}"
    count_zero_users = sum(1 for v in users_values if v["value"] == 0)
    # most values should be 0 (flat periods between increments)
    assert count_zero_users == 44, f"Expected 44 zero rate values for /users (sparse data), got {count_zero_users}"
    ...
    assert count_increment_rate == 12, f"Expected 12 increment rate values (0.0167) for /users, got {count_increment_rate}"
