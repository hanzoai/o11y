...
def test_with_steady_values_and_reset(
    ...
    assert result_values[38]["value"] == steady_value  # 38th minute is when cumulative shifts to delta
    count_of_steady_rate = sum(1 for v in result_values if v["value"] == steady_value)
    assert count_of_steady_rate >= 55  # 59 - (1 reset + 1 high rate + 1 at the beginning)
...
def test_group_by_endpoint(
    ...
    assert len(all_series) == 5, f"Expected 5 series for 5 endpoints, got {len(all_series)}"
    ...
    assert set(endpoint_values.keys()) == expected_endpoints, f"Expected endpoints {expected_endpoints}, got {set(endpoint_values.keys())}"

    # at no point rate should be negative
    for endpoint, values in endpoint_values.items():
        for v in values:
            assert v["value"] >= 0, f"Rate for {endpoint} should not be negative: {v['value']}"

    # /health: 60 data points (t01-t60), steady +10/min
    health_values = endpoint_values["/health"]
    assert len(health_values) >= 58, f"Expected >= 58 values for /health, got {len(health_values)}"
    count_steady_health = sum(1 for v in health_values if v["value"] == stable_health_value)
    assert count_steady_health >= 57, f"Expected >= 57 steady rate values ({stable_health_value}) for /health, got {count_steady_health}"
    ...
    assert count_steady_products >= 46, f"Expected >= 46 steady rate values ({stable_products_value}) for /products, got {count_steady_products}"
    ...
    for val in gap_boundary_values:
        assert 0 < val < stable_products_value, f"Gap boundary values should be between 0 and {stable_products_value}, got {val}"

    # /checkout: 61 data points (t00-t60), +1/min normal, +50/min spike at t40-t44
    checkout_values = endpoint_values["/checkout"]
    assert len(checkout_values) >= 59, f"Expected >= 59 values for /checkout, got {len(checkout_values)}"
    count_steady_checkout = sum(1 for v in checkout_values if v["value"] == stable_checkout_value)
    assert count_steady_checkout >= 53, f"Expected >= 53 steady {time_aggregation} values ({stable_checkout_value}) for /checkout, got {count_steady_checkout}"
    ...
    assert count_spike_checkout >= 4, f"Expected >= 4 spike {time_aggregation} values ({spike_checkout_value}) for /checkout, got {count_spike_checkout}"

    # spike values should be consecutive
    spike_indices = [i for i, v in enumerate(checkout_values) if v["value"] == spike_checkout_value]
    assert len(spike_indices) >= 4, f"Expected >= 4 spike indices, got {spike_indices}"
    # consecutiveness
    for i in range(1, len(spike_indices)):
        assert spike_indices[i] == spike_indices[i - 1] + 1, f"Spike indices should be consecutive, got {spike_indices}"
    ...
    assert len(orders_values) >= 58, f"Expected >= 58 values for /orders, got {len(orders_values)}"
    count_steady_orders = sum(1 for v in orders_values if v["value"] == stable_orders_value)
    assert count_steady_orders >= 55, f"Expected >= 55 steady {time_aggregation} values ({stable_orders_value}) for /orders, got {count_steady_orders}"
    ...
    assert len(non_standard_orders) >= 2, f"Expected >= 2 non-standard values due to counter reset, got {non_standard_orders}"
    ...
    assert len(high_rate_orders) >= 1, f"Expected at least one high {time_aggregation} value after counter reset, got {non_standard_orders}"

    # /users: 56 data points (t05-t60), sparse +1 every 5 minutes
    users_values = endpoint_values["/users"]
    assert len(users_values) >= 54, f"Expected >= 54 values for /users, got {len(users_values)}"
    count_zero_users = sum(1 for v in users_values if v["value"] == 0)
    # most values should be 0 (flat periods between increments)
    assert count_zero_users >= 40, f"Expected >= 40 zero {time_aggregation} values for /users (sparse data), got {count_zero_users}"
    ...
    assert count_increment_rate >= 8, f"Expected >= 8 increment {time_aggregation} values ({spike_users_value}) for /users, got {count_increment_rate}"
