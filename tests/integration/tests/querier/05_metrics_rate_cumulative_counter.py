...
def test_rate_with_steady_values_and_reset(
    ...
    assert result_values[29]["value"] == 0.0167  # i.e 2/120 i.e 29th to 31st minute changes
    assert result_values[30]["value"] == 0.133  # i.e 10/60 i.e 31st to 32nd minute changes
...
def test_rate_group_by_endpoint(
    ...
    assert count_steady_health >= 57, f"Expected >= 57 steady rate values (0.167) for /health, got {count_steady_health}"
    ...
    assert count_steady_products >= 46, f"Expected >= 46 steady rate values (0.333) for /products, got {count_steady_products}"
    ...
    for val in gap_boundary_values:
        assert 0 < val < 0.333, f"Gap boundary values should be between 0 and 0.333, got {val}"
    ...
    assert count_steady_checkout >= 53, f"Expected >= 53 steady rate values (0.0167) for /checkout, got {count_steady_checkout}"
    ...
    assert count_spike_checkout >= 4, f"Expected >= 4 spike rate values (0.833) for /checkout, got {count_spike_checkout}"

    # spike values should be consecutive
    spike_indices = [i for i, v in enumerate[Any](checkout_values) if v["value"] == 0.833]
    ...
    assert count_steady_orders >= 55, f"Expected >= 55 steady rate values (0.0833) for /orders, got {count_steady_orders}"
    ...
    assert len(high_rate_orders) >= 1, f"Expected at least one high rate value after counter reset, got {non_standard_orders}"
    ...
    assert count_zero_users >= 40, f"Expected >= 40 zero rate values for /users (sparse data), got {count_zero_users}"
    ...
    assert count_increment_rate >= 8, f"Expected >= 8 increment rate values (0.0167) for /users, got {count_increment_rate}"
