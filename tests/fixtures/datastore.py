...
def clickhouse(
...
) -> types.TestContainerDatastore:
    """
    Package-scoped fixture for Clickhouse TestContainer.
    """

    def create() -> types.TestContainerDatastore:
        version = request.config.getoption("--datastore-version")
        ...
        return types.TestContainerDatastore(
            ...
            conn=connection,
            env={
                "O11Y_TELEMETRYSTORE_DATASTORE_DSN": f"tcp://{container.username}:{container.password}@{container.get_wrapped_container().name}:{9000}",
                "O11Y_TELEMETRYSTORE_DATASTORE_USERNAME": container.username,
                "O11Y_TELEMETRYSTORE_DATASTORE_PASSWORD": container.password,
                "O11Y_TELEMETRYSTORE_DATASTORE_CLUSTER": "cluster",
            },
        )

    def delete(container: types.TestContainerDatastore) -> None:
    ...
    def restore(cache: dict) -> types.TestContainerDatastore:
        container = types.TestContainerDocker.from_cache(cache["container"])
        host_config = container.host_configs["8123"]
        env = cache["env"]

        conn = clickhouse_connect.get_client(
            user=env["O11Y_TELEMETRYSTORE_DATASTORE_USERNAME"],
            password=env["O11Y_TELEMETRYSTORE_DATASTORE_PASSWORD"],
