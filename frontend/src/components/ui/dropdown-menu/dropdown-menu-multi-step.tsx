...
import { DropdownMenuBack, DropdownMenuPortal, DropdownMenuSeparator } from './dropdown-menu';
 ...
 */
const DropdownMenuMultiStep = React.forwardRef<HTMLDivElement, DropdownMenuMultiStepProps>(
	({ children, ...props }) => {
...
>(({ primaryContent, secondaryContent, secondaryLabel, className, sideOffset = 4, ...props }, ref) => {
...
>(({ className, leftIcon, children, ...props }, ref) => {
	...
	return (
		<DropdownMenuPrimitive.Item
			...
			{...props}
		>
			{leftIcon && <span data-slot="dropdown-menu-multi-step-trigger-icon">{leftIcon}</span>}
			{children}
...
export { DropdownMenuMultiStep, DropdownMenuMultiStepContent, DropdownMenuMultiStepTrigger };
