...
// API Endpoints
const ORG_PREFERENCES_ENDPOINT = '*/v1/o11y/org/preferences/list';
const UPDATE_ORG_PREFERENCE_ENDPOINT = '*/v1/o11y/org/preferences/name/update';
const UPDATE_PROFILE_ENDPOINT = '*/v1/o11y/zeus/profiles';
const EDIT_ORG_ENDPOINT = '*/v1/o11y/orgs/me';
const INVITE_USERS_ENDPOINT = '*/v1/o11y/invite/bulk/create';
...
describe('OnboardingQuestionaire Component', () => {
	...
	describe('Step 1: Organization Details', () => {
		it('renders organization questions on initial load', () => {
			...
			expect(screen.getByText(/welcome to o11y cloud/i)).toBeInTheDocument();
		...
		it('shows migration timeline options only when specific observability tools are selected', async () => {
			...
			expect(
				screen.queryByText(/What is your timeline for migrating to Hanzo/i),
			...
			expect(
				await screen.findByText(/What is your timeline for migrating to Hanzo/i),
			).toBeInTheDocument();
		...
		it('proceeds to step 2 when next is clicked', async () => {
			...
			expect(
				await screen.findByText(/how did you first come across o11y/i, {}),
			).toBeInTheDocument();
		});
	});

	describe('Step 2: About Hanzo', () => {
		...
		it('shows other interest input when Others checkbox is selected', async () => {
			...
			expect(
				await screen.findByText(/what got you interested in o11y/i, {}),
			).toBeInTheDocument();
	...
	describe('Step 3: Optimize Hanzo Needs', () => {
