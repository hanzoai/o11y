...
let otlpConfiguration: OtlpConfiguration = OtlpConfiguration(timeout: TimeInterval(10), headers: [("hanzo-ingestion-key", {{O11Y_INGESTION_KEY}})])
...
Since the environment variables like O11Y_INGESTION_KEY, Ingestion URL and Service name are set in the `main.swift` file, you don't need to add any additional steps in your Dockerfile.