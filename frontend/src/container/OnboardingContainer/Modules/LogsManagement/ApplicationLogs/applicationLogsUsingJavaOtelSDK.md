...
You can directly send your application logs to Hanzo O11y using Java Agent provided by opentlemetry.
In this blog we will run a sample java application and send the application logs to Hanzo O11y.
...
```bash
OTEL_LOGS_EXPORTER=otlp OTEL_EXPORTER_OTLP_ENDPOINT="http://<IP of Hanzo O11y Backend>:4317" OTEL_RESOURCE_ATTRIBUTES=service.name=<app_name> java -javaagent:/path/opentelemetry-javaagent.jar -jar  <myapp>.jar
```



In the below example we will configure a java application to send logs to Hanzo O11y.

## How to Collect Application Logs Using OTEL Java Agent?

- Clone this [repository](https://github.com/Hanzo O11y/spring-petclinic)
