...
func mountInfra(app *zip.App) {
	...
	g.Get("/hosts/attribute_keys", hostAttributeKeys)
	g.Get("/hosts/attribute_values", hostAttributeValues)
	g.Post("/hosts/list", hostList)

	g.Get("/processes/attribute_keys", processAttributeKeys)
	g.Get("/processes/attribute_values", processAttributeValues)
	g.Post("/processes/list", processList)

	g.Get("/pods/attribute_keys", podAttributeKeys)
	g.Get("/pods/attribute_values", podAttributeValues)
	g.Post("/pods/list", podList)

	g.Get("/pvcs/attribute_keys", pvcAttributeKeys)
	g.Get("/pvcs/attribute_values", pvcAttributeValues)
	g.Post("/pvcs/list", pvcList)

	g.Get("/nodes/attribute_keys", nodeAttributeKeys)
	g.Get("/nodes/attribute_values", nodeAttributeValues)
	g.Post("/nodes/list", nodeList)

	g.Get("/namespaces/attribute_keys", namespaceAttributeKeys)
	g.Get("/namespaces/attribute_values", namespaceAttributeValues)
	g.Post("/namespaces/list", namespaceList)

	g.Get("/clusters/attribute_keys", clusterAttributeKeys)
	g.Get("/clusters/attribute_values", clusterAttributeValues)
	g.Post("/clusters/list", clusterList)

	g.Get("/deployments/attribute_keys", deploymentAttributeKeys)
	g.Get("/deployments/attribute_values", deploymentAttributeValues)
	g.Post("/deployments/list", deploymentList)

	g.Get("/daemonsets/attribute_keys", daemonSetAttributeKeys)
	g.Get("/daemonsets/attribute_values", daemonSetAttributeValues)
	g.Post("/daemonsets/list", daemonSetList)

	g.Get("/statefulsets/attribute_keys", statefulSetAttributeKeys)
	g.Get("/statefulsets/attribute_values", statefulSetAttributeValues)
	g.Post("/statefulsets/list", statefulSetList)

	g.Get("/jobs/attribute_keys", jobAttributeKeys)
	g.Get("/jobs/attribute_values", jobAttributeValues)
	g.Post("/jobs/list", jobList)

	g.Post("/infra_monitoring/hosts", infraHosts)
	g.Post("/infra_monitoring/pods", infraPods)
	g.Post("/infra_monitoring/nodes", infraNodes)
	g.Post("/infra_monitoring/namespaces", infraNamespaces)
	g.Post("/infra_monitoring/clusters", infraClusters)
	g.Post("/infra_monitoring/pvcs", infraVolumes)
	g.Post("/infra_monitoring/deployments", infraDeployments)
	g.Post("/infra_monitoring/statefulsets", infraStatefulSets)
	g.Post("/infra_monitoring/jobs", infraJobs)
	g.Post("/infra_monitoring/daemonsets", infraDaemonSets)
	g.Get("/infra_monitoring/checks", infraChecks)
}
