...
var (
	testCases = []struct {
		...
		metaValues         [][]any
		attrMetaValues     [][]any
		resourceMetaValues [][]any
		createTableValues  [][]any
		expectAlerts       int
		compareOperator    ruletypes.CompareOperator
		matchType          ruletypes.MatchType
		target             float64
	...
	tcThresholdRuleEvalNoRecoveryTarget = []recoveryTestCase{
		...
		},
		{
			...
			matchType:           ruletypes.AllTheTimes,
			target:              4,
			expectedAlertSample: qbtypes.TimeSeriesValue{Value: 3.5},
		},
		{
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
	...
	tcThresholdRuleEval = []recoveryTestCase{
		...
		{
			description: "Category 1: Above operator - value in recovery zone, no active alert -> no alert returned",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 90.0},
				},
				Labels: []*qbtypes.Label{
					{Key: telemetrytypes.TelemetryFieldKey{Name: "service"}, Value: "frontend"},
				},
			},
			expectAlert:     false,
		...
		// The alert fires when a value drops below the target (e.g., value < 20%, say idle cpu percentage).
		// The recovery target sits above the alert target (e.g., recovery = 30%).
		...
		// the value oscillates around the alert threshold.
		{
			...
			values: &qbtypes.TimeSeries{
				...
				Labels: []*qbtypes.Label{
					{Key: telemetrytypes.TelemetryFieldKey{Name: "service"}, Value: "backend"},
				},
			},
			expectAlert:     false,
			expectRecovery:  false,
		...
		{
			description: "Category 2: Above operator - active alert, value below target but above recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 90.0},
				},
			...
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 90.0},
			expectedTarget:         100.0,
			expectedRecoveryTarget: 80.0,
			thresholdName:          "test_threshold_above",
		},
		{
			description: "Category 2: Below operator - active alert, value above target but below recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 60.0},
				},
			...
			recoveryTarget:         func() *float64 { v := 70.0; return &v }(),
			activeAlerts:           nil, // Auto-calculate from labels+thresholdName
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 60.0},
			expectedTarget:         50.0,
			expectedRecoveryTarget: 70.0,
			thresholdName:          "test_threshold_below",
		},
		{
			description: "Category 2: NotEq operator - active alert, value equals target but not recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 1.0},
				},
		...
		{
			description: "Category 3: Above operator - active alert, value still above target -> normal firing alert",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 110.0},
				},
			...
			recoveryTarget:         func() *float64 { v := 80.0; return &v }(),
			activeAlerts:           nil, // Auto-calculate from labels+thresholdName
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 110.0},
			expectedTarget:         100.0,
			expectedRecoveryTarget: 80.0,
			thresholdName:          "test_threshold_still_alerting_above",
		},
		{
			description: "Category 3: Below operator - active alert, value still below target -> normal firing alert",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 40.0},
				},
			...
			recoveryTarget:         func() *float64 { v := 70.0; return &v }(),
			activeAlerts:           nil, // Auto-calculate from labels+thresholdName
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 40.0},
			expectedTarget:         50.0,
			expectedRecoveryTarget: 70.0,
			thresholdName:          "test_threshold_still_alerting_below",
		},
		{
			description: "Category 3: Above operator - active alert, value fully recovered (below recovery) -> alert resolved",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 75.0},
				},
		...
		{
			description: "Category 4: Wrong alert fingerprint - value in recovery zone but different active alert -> no recovery",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 90.0},
				},
		...
		{
			description: "Category 4: Correct alert fingerprint - value in recovery zone and matching active alert -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 90.0},
				},
		...
		{
			description: "Category 4: Multiple thresholds - each tracks recovery independently based on its own fingerprint",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 90.0},
				},
			...
			additionalThresholds: []struct {
				...
				{
					name:            "test_threshold_multiple_second", // imagine like a warning threshold
					target:          100.0,
					recoveryTarget:  func() *float64 { v := 80.0; return &v }(),
					matchType:       ruletypes.AtleastOnce,
					compareOperator: ruletypes.ValueIsAbove,
		...
		{
			description: "Category 4: Below operator - active alert, value fully recovered (above recovery) -> alert resolves",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
	...
	tcThresholdRuleEvalMatchPlusCompareOps = []recoveryTestCase{
		...
		{
			description: "Category 1: AtleastOnce + Above - active alert, one value in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
			...
			recoveryTarget:         func() *float64 { v := 80.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 85.0}, // first matching value
			expectedTarget:         100.0,
			expectedRecoveryTarget: 80.0,
			thresholdName:          "cat1_atleastonce_above_recovery",
		},
		{
			description: "Category 1: AtleastOnce + Below - active alert, one value in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
		...
		},
		{
			...
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 5.0},
			expectedTarget:         10.0,
			expectedRecoveryTarget: 5.0,
			thresholdName:          "cat1_atleastonce_equals_recovery",
		},
		{
			description: "Category 1: AtleastOnce + NotEquals - active alert, values equal target but not recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
			...
			recoveryTarget:         func() *float64 { v := 5.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 10.0}, // All values = 10, which != 5 (recovery condition met)
			expectedTarget:         10.0,
			expectedRecoveryTarget: 5.0,
			thresholdName:          "cat1_atleastonce_noteq_recovery",
		},
		{
			description: "Category 1: AtleastOnce + OutsideBounds - active alert, |value| in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
			...
			expectAlert:            true,
			expectRecovery:         true,
			compareOperator:        ruletypes.ValueOutsideBounds,
			matchType:              ruletypes.AtleastOnce,
			target:                 100.0,
			recoveryTarget:         func() *float64 { v := 80.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 85.0},
			expectedTarget:         100.0,
			expectedRecoveryTarget: 80.0,
			thresholdName:          "cat1_atleastonce_outsidebounds_recovery",
		},
		{
			description: "Category 1: AtleastOnce + OutsideBounds - active alert, negative value in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
		...
		{
			description: "Category 2: AllTheTimes + Above - active alert, all values in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
			...
			recoveryTarget:         func() *float64 { v := 80.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 82.0}, // min value for Above + AllTheTimes
			expectedTarget:         100.0,
			expectedRecoveryTarget: 80.0,
			thresholdName:          "cat2_allthetimes_above_recovery",
		},
		{
			description: "Category 2: AllTheTimes + Above - active alert, one value below recovery -> no recovery",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 85.0}, // in recovery zone
		...
		{
			description: "Category 2: AllTheTimes + Below - active alert, all values in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
			...
			recoveryTarget:         func() *float64 { v := 70.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 65.0}, // max value for Below + AllTheTimes
			expectedTarget:         50.0,
			expectedRecoveryTarget: 70.0,
			thresholdName:          "cat2_allthetimes_below_recovery",
		},
		{
			description: "Category 2: AllTheTimes + Equals - active alert, all values equal recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 5.0},
					{Value: 5.0},
		...
		{
			description: "Category 2: AllTheTimes + NotEquals - active alert, all values equal target but not recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
			...
			expectAlert:            true,
			expectRecovery:         true,
			compareOperator:        ruletypes.ValueIsNotEq, // NotEquals
			matchType:              ruletypes.AllTheTimes,  // AllTheTimes
			target:                 10.0,
			recoveryTarget:         func() *float64 { v := 5.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 10.0}, // All equal target, all != recovery
			expectedTarget:         10.0,
			expectedRecoveryTarget: 5.0,
		...
		{
			description: "Category 3: OnAverage + Above - active alert, avg in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 85.0},
					{Value: 90.0},
			...
			recoveryTarget:         func() *float64 { v := 80.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 86.66666666666667},
			expectedTarget:         100.0,
			expectedRecoveryTarget: 80.0,
			thresholdName:          "cat3_onaverage_above_recovery",
		},
		{
			description: "Category 3: OnAverage + Below - active alert, avg in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 60.0},
					{Value: 65.0},
		...
		{
			description: "Category 3: OnAverage + Equals - active alert, avg equals recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 4.0},
					{Value: 5.0},
			...
			expectAlert:            true,
			expectRecovery:         true,
			compareOperator:        ruletypes.ValueIsEq, // Equals
			matchType:              ruletypes.OnAverage, // OnAverage
			target:                 10.0,
			recoveryTarget:         func() *float64 { v := 5.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 5.0},
		...
		{
			description: "Category 3: OnAverage + NotEquals - active alert, avg equals target but not recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 8.0},
					{Value: 10.0},
			...
			recoveryTarget:         func() *float64 { v := 5.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 10.0}, // avg = 10.0
			expectedTarget:         10.0,
			expectedRecoveryTarget: 5.0,
			thresholdName:          "cat3_onaverage_noteq_recovery",
		},
		{
			description: "Category 3: OnAverage + OutsideBounds - active alert, avg |value| in recovery zone -> IsRecovering=false",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: -90.0},
					{Value: 85.0},
		...
		{
			description: "Category 4: InTotal + Above - active alert, sum in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 30.0},
					{Value: 35.0},
		...
		{
			description: "Category 4: InTotal + Below - active alert, sum in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 20.0},
					{Value: 25.0},
		...
		{
			description: "Category 4: InTotal + Equals - active alert, sum equals recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 1.0},
					{Value: 2.0},
		...
		{
			description: "Category 4: InTotal + NotEquals - active alert, sum equals target but not recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
					{Value: 3.0},
					{Value: 3.0},
			...
			recoveryTarget:         func() *float64 { v := 5.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 10.0}, // sum = 10.0
			expectedTarget:         10.0,
			expectedRecoveryTarget: 5.0,
		...
		{
			description: "Category 5: Last + Above - active alert, last value in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
		...
		{
			description: "Category 5: Last + Below - active alert, last value in recovery zone -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
		...
		{
			description: "Category 5: Last + Equals - active alert, last value equals recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
		...
		{
			description: "Category 5: Last + NotEquals - active alert, last value equals target but not recovery -> IsRecovering=true",
			values: &qbtypes.TimeSeries{
				Values: []*qbtypes.TimeSeriesValue{
			...
			recoveryTarget:         func() *float64 { v := 5.0; return &v }(),
			activeAlerts:           nil,
			expectedAlertSample:    qbtypes.TimeSeriesValue{Value: 10.0}, // last = 10.0
			expectedTarget:         10.0,
			expectedRecoveryTarget: 5.0,
			...
			recoveryTestCase: recoveryTestCase{
				...
				additionalThresholds: []struct {
				...
				}{
					{
						...
						matchType:       ruletypes.AtleastOnce,  // AtleastOnce
						compareOperator: ruletypes.ValueIsAbove, // Above
					},
				},
				activeAlerts: nil,
			},
			...
			recoveryTestCase: recoveryTestCase{
				...
				additionalThresholds: []struct {
				...
				}{
					{
						...
						target:          30.0,
						recoveryTarget:  func() *float64 { v := 40.0; return &v }(),
						matchType:       ruletypes.Last,         // Last
						compareOperator: ruletypes.ValueIsBelow, // Below
					},
				},
				activeAlerts: nil,
			},
			...
			recoveryTestCase: recoveryTestCase{
				...
				additionalThresholds: []struct {
				...
				}{
					{
						...
						recoveryTarget:  func() *float64 { v := 100.0; return &v }(),
						matchType:       ruletypes.AtleastOnce,  // AtleastOnce
						compareOperator: ruletypes.ValueIsBelow, // CompareOp: ValueIsBelow (2)
					},
				},
				activeAlerts: nil,
			},
