...
func (r *DatastoreReader) UpdateLogField(ctx context.Context, field *model.UpdateField) *model.ApiError {
	...
	colname := utils.GetDatastoreColumnNameV2(field.Type, field.DataType, field.Name)
...
func (r *DatastoreReader) GetMetricAttributeKeys(ctx context.Context, orgID valuer.UUID, req *v3.FilterAttributeKeyRequest) (*v3.FilterAttributeKeyResponse, error) {
	ctx = ctxtypes.NewContextWithCommentVals(ctx, map[string]string{
		instrumentationtypes.TelemetrySignal:  telemetrytypes.SignalMetrics.StringValue(),
		instrumentationtypes.CodeNamespace:    "datastore-reader",
...
func (r *DatastoreReader) GetMetricAttributeValues(ctx context.Context, orgID valuer.UUID, req *v3.FilterAttributeValueRequest) (*v3.FilterAttributeValueResponse, error) {
...
func (r *DatastoreReader) GetMetricMetadata(ctx context.Context, orgID valuer.UUID, metricName, serviceName string) (*v3.MetricMetadataResponse, error) {
	...
	if metricType == string(v3.MetricTypeHistogram) {
		...
		var query string
		if reductionEnabled {
			// `le` is a protected label that survives reduction
			query = fmt.Sprintf(`
				SELECT le FROM (
...
func (r *DatastoreReader) GetLatestReceivedMetric(
	...
	if reductionEnabled {
		...
		from (
			SELECT metric_name, labels, unix_milli FROM %s.%s WHERE %s
			UNION ALL
			SELECT metric_name, labels, unix_milli FROM %s.%s WHERE %s
		)
...
func isColumn(tableStatement, attrType, field, datType string) bool {
	name := fmt.Sprintf("`%s`", utils.GetDatastoreColumnNameV2(attrType, datType, field))
...
func (r *DatastoreReader) UpdateMetricsMetadata(ctx context.Context, orgID valuer.UUID, req *model.UpdateMetricsMetadata) *model.ApiError {
	...
	if req.MetricType == v3.MetricTypeHistogram {
		labels := []string{"le"}
		hasLabels, apiError := r.CheckForLabelsInMetric(ctx, orgID, req.MetricName, labels)
		if apiError != nil {
			return apiError
		}
		if !hasLabels {
...
func (r *DatastoreReader) CheckForLabelsInMetric(ctx context.Context, orgID valuer.UUID, metricName string, labels []string) (bool, *model.ApiError) {
	ctx = ctxtypes.NewContextWithCommentVals(ctx, map[string]string{
		instrumentationtypes.TelemetrySignal:  telemetrytypes.SignalMetrics.StringValue(),
		instrumentationtypes.CodeNamespace:    "datastore-reader",
...
func (r *DatastoreReader) GetUpdatedMetricsMetadata(ctx context.Context, orgID valuer.UUID, metricNames ...string) (map[string]*model.UpdateMetricsMetadata, *model.ApiError) {
	...
	if len(missingMetrics) > 0 {
		...
		if err != nil {
			return cachedMetadata, &model.ApiError{Typ: "DatastoreErr", Err: fmt.Errorf("error querying metrics metadata: %v", err)}
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			if err := rows.Scan(
				...
				return cachedMetadata, &model.ApiError{Typ: "DatastoreErr", Err: fmt.Errorf("error scanning metrics metadata: %v", err)}
			}
	...
	if len(stillMissing) > 0 {
		...
		for rows.Next() {
			...
			if err := rows.Scan(
				...
				return cachedMetadata, &model.ApiError{Typ: "DatastoreErr", Err: fmt.Errorf("error scanning fallback metadata: %v", err)}
			}
