...
func getParamsForTopItems(start, end int64) (int64, string, string) {
	...
	if end-start < time.Hour.Milliseconds() {
		...
		step = 5 * 60
		timeSeriesTableName = telemetrymetrics.SeriesLocalTableName
		samplesTableName = telemetrymetrics.Metric5mTableName
	} else if end-start < time.Hour.Milliseconds()*6 {
		// 15 minute aggregation for any query less than 6 hours
		step = 15 * 60
		timeSeriesTableName = telemetrymetrics.Series6hLocalTableName
		samplesTableName = telemetrymetrics.Metric5mTableName
	} else if end-start < time.Hour.Milliseconds()*24 {
		// 1 hour aggregation for any query less than 1 day
		step = 60 * 60
		timeSeriesTableName = telemetrymetrics.Series1dLocalTableName
		samplesTableName = telemetrymetrics.Metric30mTableName
	} else if end-start < time.Hour.Milliseconds()*7 {
		// 6 hours aggregation for any query less than 1 week
		step = 6 * 60 * 60
		timeSeriesTableName = telemetrymetrics.Series1wLocalTableName
		samplesTableName = telemetrymetrics.Metric30mTableName
	} else {
...
// PrepareTimeseriesFilterQuery reads the LOCAL series table, because each fingerprint
// lands on one shard. Reading attribute values across every shard needs the
// distributed table instead — and on this deployment there is no Distributed wrapper,
// so the local table already spans everything and the query is used as written.
