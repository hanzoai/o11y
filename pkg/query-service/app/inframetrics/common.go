...
func getParamsForTopItems(start, end int64) (int64, string, string) {
	...
	if end-start < time.Hour.Milliseconds() {
		...
		step = 5 * 60
		timeSeriesTableName = constants.O11Y_TIMESERIES_v4_LOCAL_TABLENAME
		samplesTableName = constants.O11Y_SAMPLES_V4_AGG_5M_TABLENAME
	} else if end-start < time.Hour.Milliseconds()*6 {
		// 15 minute aggregation for any query less than 6 hours
		step = 15 * 60
		timeSeriesTableName = constants.O11Y_TIMESERIES_v4_6HRS_LOCAL_TABLENAME
		samplesTableName = constants.O11Y_SAMPLES_V4_AGG_5M_TABLENAME
	} else if end-start < time.Hour.Milliseconds()*24 {
		// 1 hour aggregation for any query less than 1 day
		step = 60 * 60
		timeSeriesTableName = constants.O11Y_TIMESERIES_v4_1DAY_LOCAL_TABLENAME
		samplesTableName = constants.O11Y_SAMPLES_V4_AGG_30M_TABLENAME
	} else if end-start < time.Hour.Milliseconds()*7 {
		// 6 hours aggregation for any query less than 1 week
		step = 6 * 60 * 60
		timeSeriesTableName = constants.O11Y_TIMESERIES_v4_1WEEK_LOCAL_TABLENAME
		samplesTableName = constants.O11Y_SAMPLES_V4_AGG_30M_TABLENAME
	} else {
