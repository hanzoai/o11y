...
func buildSpanScopeQuery(fs *v3.FilterSet) (string, error) {
	...
	for _, item := range fs.Items {
		...
		if keyName == constants.SpanSearchScopeRoot {
			...
			query = "((name, `resource_string_service$$name`) GLOBAL IN ( SELECT DISTINCT name, serviceName from " + telemetrytraces.DBName + "." + telemetrytraces.OperationTableName + " )) AND parent_span_id != '' "
			return query, nil
		} else {
...
func buildTracesQuery(start, end, step int64, mq *v3.BuilderQuery, panelType v3.PanelType, options v3.QBOptions) (string, error) {
	...
	resourceSubQuery, err := resource.BuildResourceSubQuery(telemetrytraces.DBName, telemetrytraces.SpanResourceTableName, bucketStart, bucketEnd, mq.Filters, mq.GroupBy, mq.AggregateAttribute, false)
	if err != nil {
		return "", err
	}
	...
	if mq.AggregateOperator == v3.AggregateOperatorNoOp {
		...
		if panelType == v3.PanelTypeTrace {
			...
			if !orderBySpanCount {
				withSubQuery := fmt.Sprintf(constants.TracesExplorerViewSQLSelectWithSubQuery, telemetrytraces.DBName, telemetrytraces.SpanLocalTableName, timeFilter)
				...
				query = fmt.Sprintf(constants.TracesExplorerViewSQLSelectBeforeSubQuery, telemetrytraces.DBName, telemetrytraces.SpanTableName, timeFilter, filterSubQuery) + withSubQuery + ")" + afterSubQuery
			} else {
				withSubQueryWithLimits := tracesV3.AddLimitToQuery(constants.TracesExplorerSpanCountWithSubQuery, mq.Limit)
				withSubQuery := fmt.Sprintf(withSubQueryWithLimits, telemetrytraces.DBName, telemetrytraces.SpanLocalTableName, timeFilter, filterSubQuery)
				afterSubQuery := tracesV3.AddLimitToQuery(constants.TraceExplorerSpanCountAfterSubQuery, mq.Limit)
				if mq.Offset != 0 {
					afterSubQuery = tracesV3.AddOffsetToQuery(afterSubQuery, mq.Offset)
				}
				query = fmt.Sprintf(constants.TraceExplorerSpanCountBeforeSubQuery, telemetrytraces.DBName, telemetrytraces.SpanTableName) + withSubQuery + ") " + fmt.Sprintf(afterSubQuery, telemetrytraces.DBName, telemetrytraces.SpanTableName, timeFilter)
			}
			...
			queryNoOpTmpl := fmt.Sprintf("SELECT timestamp as timestamp_datetime, span_id as spanID, trace_id as traceID,%s ", selectLabels) + "from " + telemetrytraces.DBName + "." + telemetrytraces.SpanTableName + " where %s %s" + "%s"
	...
	queryTmpl = queryTmpl + selectLabels +
		" %s as value " +
		"from " + telemetrytraces.DBName + "." + telemetrytraces.SpanTableName +
		" where " + timeFilter + "%s" +
		"%s%s" +
		"%s"
