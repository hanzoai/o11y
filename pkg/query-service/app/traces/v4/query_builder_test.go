	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := getDatastoreTracesColumnType(tt.args.columnType); got != tt.want {
				t.Errorf("GetDatastoreTracesColumnType() = %v, want %v", got, tt.want)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := getDatastoreTracesColumnDataType(tt.args.columnDataType); got != tt.want {
				t.Errorf("getDatastoreTracesColumnDataType() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func Test_orderByAttributeKeyTags(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				panelType: v3.PanelTypeGraph,
				items:     []v3.OrderBy{{ColumnName: "name", Order: "ASC"}, {ColumnName: constants.O11yOrderByValue, Order: "DESC"}},
				tags:      []v3.AttributeKey{{Key: "name", DataType: v3.AttributeKeyDataTypeString, Type: v3.AttributeKeyTypeTag}},
			},
...
func Test_buildTracesQuery(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: "SELECT  attributes_string['http.method'] as `http.method`, toFloat64(count()) as value from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') " +
		...
		},
		{
			...
			want: "SELECT  resources_number['host'] as `host`, toFloat64(count()) as value from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') " +
				"AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458) AND attributes_number['bytes'] > 100 AND " +
				"(resource_fingerprint GLOBAL IN (SELECT fingerprint FROM o11y_traces.distributed_traces_v3_resource WHERE (seen_at_ts_bucket_start >= 1680064560) AND " +
		...
		},
		{
			...
			want: "SELECT timestamp as timestamp_datetime, span_id as spanID, trace_id as traceID, name as `name` from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') " +
		...
		},
		{
			...
			want: "SELECT timestamp as timestamp_datetime, span_id as spanID, trace_id as traceID, name as `name` from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') " +
				"AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458)  AND ((name, `resource_string_service$$name`) GLOBAL IN ( SELECT DISTINCT name, serviceName from o11y_traces.distributed_top_level_operations )) AND parent_span_id != ''  order by timestamp ASC",
		},
		{
		...
		},
		{
			...
			want: "SELECT timestamp as timestamp_datetime, span_id as spanID, trace_id as traceID, name as `name` from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') " +
				"AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458)  AND (resource_fingerprint GLOBAL IN (SELECT fingerprint FROM o11y_traces.distributed_traces_v3_resource WHERE (seen_at_ts_bucket_start >= 1680064560) AND (seen_at_ts_bucket_start <= 1680066458) AND simpleJSONExtractString(labels, 'service.name') = 'cartservice' AND labels like '%service.name\":\"cartservice%')) AND parent_span_id = ''  order by timestamp ASC",
		},
		{
		...
		},
		{
			...
			want: "SELECT subQuery.serviceName as `subQuery.serviceName`, subQuery.name as `subQuery.name`, count() AS span_count, subQuery.durationNano as `subQuery.durationNano`, subQuery.traceID FROM " +
				"(SELECT traceID AS dist_traceID, timestamp, ts_bucket_start FROM o11y_traces.distributed_o11y_index_v3 WHERE (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458) AND attributes_string['method'] = 'GET' " +
				"AND (resource_fingerprint GLOBAL IN (SELECT fingerprint FROM o11y_traces.distributed_traces_v3_resource WHERE (seen_at_ts_bucket_start >= 1680064560) AND (seen_at_ts_bucket_start <= 1680066458) AND simpleJSONExtractString(labels, 'service.name') = 'myService' AND labels like '%service.name\":\"myService%'))) as dist_table " +
				"INNER JOIN " +
				"( SELECT * FROM (SELECT traceID, durationNano, serviceName, name FROM o11y_traces.o11y_index_v3 WHERE parentSpanID = '' AND (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458) " +
		...
		},
		{
			...
			want: "SELECT serviceName, name, subQuery.span_count as span_count, durationNano, trace_id as traceID from o11y_traces.distributed_o11y_index_v3 GLOBAL INNER JOIN " +
				"( SELECT * FROM (SELECT trace_id, count() as span_count FROM o11y_traces.o11y_index_v3 WHERE (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458)  " +
				"AND attributes_string['method'] = 'GET' AND (resource_fingerprint GLOBAL IN (SELECT fingerprint FROM o11y_traces.distributed_traces_v3_resource WHERE " +
				...
				"GROUP BY trace_id ORDER BY span_count DESC LIMIT 1 BY trace_id LIMIT 100) AS inner_subquery ) AS subQuery ON o11y_traces.distributed_o11y_index_v3.trace_id = subQuery.trace_id " +
		...
		},
		{
			...
			want: "SELECT  toFloat64(count(distinct(name))) as value from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND " +
		...
		},
		{
			...
			want: "SELECT  max(toUnixTimestamp64Nano(timestamp)) as value from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND " +
		...
		},
		{
			...
			want: "SELECT toStartOfInterval(timestamp, INTERVAL 60 SECOND) AS ts, count()/60.000000 as value from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1745315470000000000' AND " +
		...
		},
		{
			...
			want: "SELECT  count()/3600.000000 as value from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1745315470000000000' AND " +
...
func TestPrepareTracesQuery(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: "SELECT `function` from (SELECT attributes_string['function'] as `function`, toFloat64(count(distinct(name))) as value from o11y_traces.distributed_o11y_index_v3 " +
		...
		},
		{
			...
			want: "SELECT  attributes_string['function'] as `function`, toFloat64(count(distinct(name))) as value from o11y_traces.distributed_o11y_index_v3 where " +
		...
		},
		{
			...
			want: "SELECT `function`,`service.name` from (SELECT `attribute_string_function` as `function`, `resource_string_service$$name` as `service.name`, toFloat64(count(distinct(name))) as value " +
				"from o11y_traces.distributed_o11y_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458) " +
				"AND attributes_number['line'] = 100 AND (resource_fingerprint GLOBAL IN (SELECT fingerprint FROM o11y_traces.distributed_traces_v3_resource WHERE " +
		...
		},
		{
			...
			want: "SELECT  `attribute_string_function` as `function`, resource_string_service$$name as `serviceName`, toFloat64(count(distinct(name))) as value from o11y_traces.distributed_o11y_index_v3 " +
				...
				"AND (resource_fingerprint GLOBAL IN (SELECT fingerprint FROM o11y_traces.distributed_traces_v3_resource WHERE (seen_at_ts_bucket_start >= 1680064560) AND (seen_at_ts_bucket_start <= 1680066458) " +
