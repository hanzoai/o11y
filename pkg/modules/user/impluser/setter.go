...
func (module *setter) CreateBulkInvite(ctx context.Context, orgID valuer.UUID, identityID valuer.UUID, identityEmail valuer.Email, bulkInvites *types.PostableBulkInviteRequest) ([]*types.Invite, error) {
	...
	if err := module.store.RunInTx(ctx, func(ctx context.Context) error {
		for idx, invite := range bulkInvites.Invites {
			...
			err = module.createUserWithoutGrant(ctx, newUser, root.WithRoleNames([]string{authtypes.MustGetHanzoO11yManagedRoleFromExistingRole(invite.Role)}))
			if err != nil {
				return err
			}
	...
	for idx, userWithToken := range newUsersWithResetToken {
		...
		if err := module.emailing.SendHTML(ctx, userWithToken.User.Email.String(), "You're Invited to Join Hanzo O11y", emailtypes.TemplateNameInvitationEmail, map[string]any{
...
func (module *setter) UpdateUserDeprecated(ctx context.Context, orgID valuer.UUID, id string, user *types.DeprecatedUser) (*types.DeprecatedUser, error) {
	...
	if roleChange {
		err = module.authz.ModifyGrant(ctx,
			orgID,
			[]string{authtypes.MustGetHanzoO11yManagedRoleFromExistingRole(existingUser.Role)},
			[]string{authtypes.MustGetHanzoO11yManagedRoleFromExistingRole(user.Role)},
	...
	err = module.store.RunInTx(ctx, func(ctx context.Context) error {
		if roleChange {
			...
			// create new ones
			if err := module.createUserRoleEntries(ctx, existingUser.OrgID, existingUser.ID, []string{authtypes.MustGetHanzoO11yManagedRoleFromExistingRole(user.Role)}); err != nil {
				return err
			}
		}
...
func (module *setter) CreateFirstUser(ctx context.Context, organization *types.Organization, name string, email valuer.Email, passwd string) (*types.User, error) {
	...
	roleNames := []string{authtypes.HanzoO11yAdminRoleName}
