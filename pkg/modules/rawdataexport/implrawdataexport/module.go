...
func (m *Module) ExportRawData(ctx context.Context, orgID valuer.UUID, rangeRequest *qbtypes.QueryRangeRequest, doneChan chan any) (chan *qbtypes.RawRow, chan error) {
	...
	go func() {
		...
		ctx := ctxtypes.SetDatastoreMaxThreads(ctx, DatastoreExportRawDataMaxThreads)
		// Set datastore timeout
		contextWithTimeout, cancel := context.WithTimeout(ctx, DatastoreExportRawDataTimeout)
		defer cancel()
		defer close(errChan)
		defer close(rowChan)
