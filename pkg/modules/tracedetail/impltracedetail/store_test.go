...
func TestGetFlamegraphSpans(t *testing.T) {
	flamegraphCols := "span_id, any(parent) AS parent_span_id, any(time) AS timestamp, any(duration) AS duration_nano, any(toBool(status = 'error')) AS has_error, any(name) AS name, any(arrayFilter(x -> x != '', [attributes['events']])) AS events, any(attributes) AS attributes_string, any(CAST(map(), 'Map(String, Float64)')) AS attributes_number, any(CAST(map(), 'Map(String, Bool)')) AS attributes_bool, any(map('service.name', toString(service))) AS resources_string"
	baseSQL := "SELECT " + flamegraphCols + " FROM event.span WHERE trace_id = ? AND ts_bucket_start >= ? AND ts_bucket_start <= ? GROUP BY span_id ORDER BY timestamp ASC, name ASC"
	withSpanIDsSQL := "SELECT " + flamegraphCols + " FROM event.span WHERE trace_id = ? AND ts_bucket_start >= ? AND ts_bucket_start <= ? AND span_id IN (?, ?) GROUP BY span_id ORDER BY timestamp ASC, name ASC"
