...
// ExplainPlanNode is a node in Datastore's `EXPLAIN json = 1, indexes = 1`
...
func RunExplainEstimate(ctx context.Context, conn datastore.Conn, stmt string, args ...any) ([]telemetrystoretypes.EstimateEntry, error) {
...
func RunExplainPlan(ctx context.Context, conn datastore.Conn, stmt string, args ...any) error {
	if err := ValidateExplainStatement(stmt); err != nil {
		return err
	}
...
func RunExplainIndexes(ctx context.Context, conn datastore.Conn, stmt string, args ...any) (telemetrystoretypes.Granules, bool, error) {
	if err := ValidateExplainStatement(stmt); err != nil {
