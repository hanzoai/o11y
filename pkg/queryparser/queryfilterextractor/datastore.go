...
func (e *DatastoreFilterExtractor) Extract(query string) (*FilterResult, error) {
	...
	topLevelQueries := make(map[*dsparser.SelectQuery]bool)

	// Process all statements
	for _, stmt := range stmts {
		selectQuery, ok := stmt.(*dsparser.SelectQuery)
		if !ok {
			continue
		}
		...
		dsparser.Walk(selectQuery, func(node dsparser.Expr) bool {
	...
	cteMap := make(map[string]*dsparser.SelectQuery)
	...
	visited := make(map[*dsparser.SelectQuery]bool)
	for query := range topLevelQueries {
...
func (e *DatastoreFilterExtractor) fillMetricNamesFromExpr(node dsparser.Expr, metricNames map[string]bool) {

	switch n := node.(type) {
	case *dsparser.BinaryOperation:
		e.fillMetricFromBinaryOp(n, metricNames)
	}
}

// fillMetricFromBinaryOp extracts metrics from binary operations.
func (e *DatastoreFilterExtractor) fillMetricFromBinaryOp(op *dsparser.BinaryOperation, metricNames map[string]bool) {
...
func (e *DatastoreFilterExtractor) fillMetricWithBinaryOpConditions(op *dsparser.BinaryOperation, valueExpr dsparser.Expr, metricNames map[string]bool) {
	switch op.Operation {
	case dsparser.TokenKindSingleEQ, dsparser.TokenKindDoubleEQ:
		...
		if fn, ok := valueExpr.(*dsparser.FunctionExpr); ok {
	...
	case "IN", "GLOBAL IN":
		...
		// Skip if value side is a function call (function-wrapped literals are ignored, test case: CH59)
		if _, ok := valueExpr.(*dsparser.FunctionExpr); !ok {
			e.extractInValues(valueExpr, metricNames)
		}
	}
}
...
func (e *DatastoreFilterExtractor) extractStringLiteral(expr dsparser.Expr) string {
	switch ex := expr.(type) {
	case *dsparser.StringLiteral:
		return ex.Literal
	}
	return ""
}
...
func (e *DatastoreFilterExtractor) extractInValues(expr dsparser.Expr, metricNames map[string]bool) {
	// Find all string literals in the expression
	strLits := dsparser.FindAll(expr, func(node dsparser.Expr) bool {
		// metric_name passed in `in` condition will be string literal.
		_, ok := node.(*dsparser.StringLiteral)
		return ok
	})

	for _, strLitNode := range strLits {
		if strLit, ok := strLitNode.(*dsparser.StringLiteral); ok {
...
func (e *DatastoreFilterExtractor) extractGroupByColumns(query *dsparser.SelectQuery, cteMap map[string]*dsparser.SelectQuery, visited map[*dsparser.SelectQuery]bool) ([]ColumnInfo, error) {
	if visited[query] {
		return nil, nil
	}
	...
	if hasGroupBy {
		...
		for groupByCol := range tempGroupBy {
			...
			originVisited := make(map[*dsparser.SelectQuery]bool)
...
func (e *DatastoreFilterExtractor) fillGroupsFromGroupByClause(groupByClause *dsparser.GroupByClause, groupBy map[string]bool) {
	...
	exprListNode, foundList := dsparser.Find(groupByClause, func(node dsparser.Expr) bool {
		_, ok := node.(*dsparser.ColumnExprList)
		return ok
	})
	...
	if exprList, ok := exprListNode.(*dsparser.ColumnExprList); ok {
...
//   - "`m.region`" -> "`m.region`"
func (e *DatastoreFilterExtractor) extractColumnStrByExpr(expr dsparser.Expr) string {
	if expr == nil {
		return ""
	}
...
func (e *DatastoreFilterExtractor) getColumnName(expr dsparser.Expr) string {
	switch ex := expr.(type) {
	case *dsparser.Ident:
		return ex.Name
	case *dsparser.Path:
...
func (e *DatastoreFilterExtractor) extractSourceQuery(query *dsparser.SelectQuery, cteMap map[string]*dsparser.SelectQuery) *dsparser.SelectQuery {
	if query.From == nil {
		return nil
	}

	// Find the FROM clause and extract the source
	fromExprs := dsparser.FindAll(query.From, func(node dsparser.Expr) bool {
		switch node.(type) {
		case *dsparser.Ident, *dsparser.SelectQuery:
			return true
		}
		return false
	})
...
func (e *DatastoreFilterExtractor) extractColumnOrigin(
	columnName string,
	query *dsparser.SelectQuery,
	cteMap map[string]*dsparser.SelectQuery,
	visited map[*dsparser.SelectQuery]bool,
) string {
	if query == nil {
		return columnName
	}
	...
	selectItems := dsparser.FindAll(query, func(node dsparser.Expr) bool {
		_, ok := node.(*dsparser.SelectItem)
		return ok
	})

	// extractOriginFromSelectItem extracts the origin from a SelectItem
	extractOriginFromSelectItem := func(selectItem *dsparser.SelectItem) *string {
	...
	for _, itemNode := range selectItems {
		if selectItem, ok := itemNode.(*dsparser.SelectItem); ok {
...
func (e *DatastoreFilterExtractor) extractFullExpression(expr dsparser.Expr) string {
	if expr == nil {
		return ""
	}
	return expr.String()
...
func (e *DatastoreFilterExtractor) isSimpleColumnReference(expr dsparser.Expr) bool {
	if expr == nil {
		return false
	}
	switch ex := expr.(type) {
...
func (e *DatastoreFilterExtractor) extractSelectColumns(query *dsparser.SelectQuery) map[string]string {
...
func (e *DatastoreFilterExtractor) extractSelectItemName(selectItem *dsparser.SelectItem) string {
...
func (e *DatastoreFilterExtractor) extractSelectItemAlias(selectItem *dsparser.SelectItem) string {
...
func (e *DatastoreFilterExtractor) buildCTEMap(query *dsparser.SelectQuery, cteMap map[string]*dsparser.SelectQuery) {
...
func (e *DatastoreFilterExtractor) extractCTEName(cte *dsparser.CTEStmt) string {
...
func (e *DatastoreFilterExtractor) extractCTEQuery(cte *dsparser.CTEStmt) *dsparser.SelectQuery {
	if cte == nil || cte.Alias == nil {
		return nil
	}

	// The Alias field should contain a SelectQuery
	if selectQuery, ok := cte.Alias.(*dsparser.SelectQuery); ok {
		return selectQuery
	}
...
func (e *DatastoreFilterExtractor) buildCTEMapFromExpr(expr dsparser.Expr, cteMap map[string]*dsparser.SelectQuery) {

	// Walk through all nodes to find SelectQuery nodes that might contain CTEs
	dsparser.Walk(expr, func(node dsparser.Expr) bool {
		switch n := node.(type) {
		case *dsparser.SelectQuery:
		...
		case *dsparser.TableExpr:
			if n.Expr != nil {
				e.buildCTEMapFromExpr(n.Expr, cteMap)
			}
		case *dsparser.JoinTableExpr:
			if n.Table != nil {
