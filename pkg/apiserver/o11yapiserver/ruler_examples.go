...
func postableRuleExamples() []handler.OpenAPIExample {
	...
	return []handler.OpenAPIExample{
		...
		},
		{
			...
			Description: "Anomaly rules are not yet supported under schemaVersion v2alpha1, so this example uses the v1 shape. Wraps a builder query in the `anomaly` function with daily seasonality O11y compares each point against the forecast for that time of day. Fires when the anomaly score stays below the threshold for the entire window; `requireMinPoints` guards against noisy intervals.",
			Value: map[string]any{
		...
		},
		{
			...
			Description: "Builder query against the traces signal with p99(duration_nano). The series unit is ns (compositeQuery.unit), the target is in seconds (threshold.targetUnit) O11y converts before comparing. Canonical shape when series and target live in different units.",
			Value: map[string]any{
