...
func (provider *provider) addQuerierRoutes(router *mux.Router) error {
	if err := router.Handle("/v1/o11y/v5/query_range", handler.New(provider.authzMiddleware.ViewAccess(provider.querierHandler.QueryRange), handler.OpenAPIDef{
		ID:                 "QueryRangeV5",
		Tags:               []string{"querier"},
		Summary:            "Query range",
	...
	if err := router.Handle("/v1/o11y/v5/substitute_vars", handler.New(provider.authzMiddleware.ViewAccess(provider.querierHandler.ReplaceVariables), handler.OpenAPIDef{
		ID:                  "ReplaceVariables",
		Tags:                []string{"querier"},
