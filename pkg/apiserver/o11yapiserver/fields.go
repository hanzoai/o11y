...
func (provider *provider) addFieldsRoutes(router *mux.Router) error {
	if err := router.Handle("/v1/o11y/v1/fields/keys", handler.New(provider.authzMiddleware.ViewAccess(provider.fieldsHandler.GetFieldsKeys), handler.OpenAPIDef{
		ID:                  "GetFieldsKeys",
		Tags:                []string{"fields"},
	...
	if err := router.Handle("/v1/o11y/v1/fields/values", handler.New(provider.authzMiddleware.ViewAccess(provider.fieldsHandler.GetFieldsValues), handler.OpenAPIDef{
		ID:                  "GetFieldsValues",
		Tags:                []string{"fields"},
