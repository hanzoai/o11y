...
func (provider *provider) addOrgRoutes(router *mux.Router) error {
	if err := router.Handle("/v1/o11y/v2/orgs/me", handler.New(provider.authzMiddleware.AdminAccess(provider.orgHandler.Get), handler.OpenAPIDef{
	...
	if err := router.Handle("/v1/o11y/v2/orgs/me", handler.New(provider.authzMiddleware.AdminAccess(provider.orgHandler.Update), handler.OpenAPIDef{
