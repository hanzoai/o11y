...
func TestClient_QuerySamples(t *testing.T) {
	...
	cols = append(cols, dsmock.ColumnType{Name: "metric_name", Type: "String"})
	cols = append(cols, dsmock.ColumnType{Name: "fingerprint", Type: "UInt64"})
	cols = append(cols, dsmock.ColumnType{Name: "unix_milli", Type: "Int64"})
	cols = append(cols, dsmock.ColumnType{Name: "value", Type: "Float64"})
	cols = append(cols, dsmock.ColumnType{Name: "flags", Type: "UInt32"})
	tests := []struct {
		name               string
		start              int64
		end                int64
		...
		subQuery           string
		args               []any
		setupMock          func(mock datastoremock.Conn, args ...any)
...
func TestClient_getFingerprintsFromDatastoreQuery(t *testing.T) {
	...
	tests := []struct {
		...
		subQuery   string
		args       []any
		setupMock  func(m datastoremock.Conn, args ...any)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			store := telemetrystoretest.New(
				telemetrystore.Config{Provider: "datastore"},
			...
			got, err := c.getFingerprintsFromDatastoreQuery(ctx, tc.subQuery, tc.args)
			if tc.wantErr {
				require.Error(t, err)
				require.Nil(t, got)
				return
