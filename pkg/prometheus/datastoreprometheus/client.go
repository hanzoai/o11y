...
func (client *client) Read(ctx context.Context, query *prompb.Query, sortSeries bool) (storage.SeriesSet, error) {
	...
	datastoreQuery, args, err := client.queryToDatastoreQuery(ctx, query, metricName, false)
	if err != nil {
		return nil, err
	}

	fingerprints, err := client.getFingerprintsFromDatastoreQuery(ctx, datastoreQuery, args)
	if err != nil {
		return nil, err
	}
	...
	datastoreSubQuery, args, err := client.queryToDatastoreQuery(ctx, query, metricName, true)
	if err != nil {
		return nil, err
	}

	res := new(prompb.QueryResult)
	timeseries, err := client.querySamples(ctx, int64(query.StartTimestampMs), int64(query.EndTimestampMs), fingerprints, metricName, datastoreSubQuery, args)
	if err != nil {
		return nil, err
	}
...
func (client *client) queryToDatastoreQuery(_ context.Context, query *prompb.Query, metricName string, subQuery bool) (string, []any, error) {
	var datastoreQuery string
	...
	datastoreQuery = fmt.Sprintf(`SELECT %s FROM %s.%s WHERE %s GROUP BY fingerprint`, selectString, databaseName, tableName, whereClause)

	return datastoreQuery, args, nil
}
...
func buildSamplesQuery(start int64, end int64, metricName string, subQuery string, args []any) (string, []any) {
	...
	query := fmt.Sprintf(`
		...
		databaseName, metricTable, subQuery, strconv.Itoa(argCount+2), strconv.Itoa(argCount+3))
...
func (client *client) querySamples(ctx context.Context, start int64, end int64, fingerprints map[uint64][]prompb.Label, metricName string, subQuery string, args []any) ([]*prompb.TimeSeries, error) {
	ctx = client.withDatastorePrometheusContext(ctx, "querySamples")
...
func (client *client) withDatastorePrometheusContext(ctx context.Context, functionName string) context.Context {
	comments := map[string]string{
		instrumentationtypes.TelemetrySignal:  telemetrytypes.SignalMetrics.StringValue(),
		instrumentationtypes.CodeNamespace:    "datastore-prometheus",
