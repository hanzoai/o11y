...
func TestStatementBuilderTimeSeries(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query: "WITH __limit_cte AS (SELECT toString(multiIf(service <> ?, service, NULL)) AS `service.name`, countDistinct(multiIf(service <> ?, service, NULL)) AS __result_0 FROM event.log WHERE ((service = ? AND service <> ?) OR (attributes['http.method'] = ? AND mapContains(attributes, 'http.method') = ?)) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? GROUP BY `service.name` ORDER BY __result_0 DESC LIMIT ?) SELECT toStartOfInterval(time, INTERVAL 30 SECOND) AS ts, toString(multiIf(service <> ?, service, NULL)) AS `service.name`, countDistinct(multiIf(service <> ?, service, NULL)) AS __result_0 FROM event.log WHERE ((service = ? AND service <> ?) OR (attributes['http.method'] = ? AND mapContains(attributes, 'http.method') = ?)) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? AND (`service.name`) GLOBAL IN (SELECT `service.name` FROM __limit_cte) GROUP BY ts, `service.name`",
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query: "WITH __resource_filter AS (SELECT fingerprint FROM event.log_resource WHERE (simpleJSONExtractString(labels, 'service.name') = ? AND labels LIKE ? AND labels LIKE ?) AND seen_at_ts_bucket_start >= ? AND seen_at_ts_bucket_start <= ? GROUP BY fingerprint), __limit_cte AS (SELECT toString(multiIf(service <> ?, service, NULL)) AS `service.name`, count() AS __result_0 FROM event.log WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? GROUP BY `service.name` ORDER BY `service.name` desc LIMIT ?) SELECT toStartOfInterval(time, INTERVAL 30 SECOND) AS ts, toString(multiIf(service <> ?, service, NULL)) AS `service.name`, count() AS __result_0 FROM event.log WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? AND (`service.name`) GLOBAL IN (SELECT `service.name` FROM __limit_cte) GROUP BY ts, `service.name` ORDER BY `service.name` desc, ts desc",
				Args:  []any{"cartservice", "%service.name%", "%service.name\":\"cartservice%", uint64(1705397400), uint64(1705485600), "1705399200000000000", uint64(1705397400), "1705485600000000000", uint64(1705485600), 10, "1705399200000000000", uint64(1705397400), "1705485600000000000", uint64(1705485600)},
			},
			expectedErr: nil,
		},
...
func TestStatementBuilderListQuery(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expected: qbtypes.Statement{
				Query: "WITH __resource_filter AS (SELECT fingerprint FROM event.log_resource WHERE (simpleJSONExtractString(labels, 'service.name') = ? AND labels LIKE ? AND labels LIKE ?) AND seen_at_ts_bucket_start >= ? AND seen_at_ts_bucket_start <= ? GROUP BY fingerprint) SELECT time AS `time`, id, trace_id, span_id, toUInt32OrZero(attributes['trace_flags']) AS `trace_flags`, severity_text, severity_number, attributes['scope.name'] AS `scope_name`, attributes['scope.version'] AS `scope_version`, body, attributes AS `attributes_string`, CAST(map(), 'Map(String, Float64)') AS `attributes_number`, CAST(map(), 'Map(String, Bool)') AS `attributes_bool`, map('service.name', toString(service)) AS `resources_string`, CAST(map(), 'Map(String, String)') AS `scope_string` FROM event.log WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query: "SELECT time AS `time`, id, trace_id, span_id, toUInt32OrZero(attributes['trace_flags']) AS `trace_flags`, severity_text, severity_number, attributes['scope.name'] AS `scope_name`, attributes['scope.version'] AS `scope_version`, body, attributes AS `attributes_string`, CAST(map(), 'Map(String, Float64)') AS `attributes_number`, CAST(map(), 'Map(String, Bool)') AS `attributes_bool`, map('service.name', toString(service)) AS `resources_string`, CAST(map(), 'Map(String, String)') AS `scope_string` FROM event.log WHERE ((match(attributes['materialized.key.name'], ?) AND mapContains(attributes, 'materialized.key.name') = ?) OR (attributes['materialized.key.name'] = ? AND mapContains(attributes, 'materialized.key.name') = ?)) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? ORDER BY attributes['materialized.key.name'] AS `materialized.key.name` desc LIMIT ?",
...
func TestStatementBuilderListQueryResourceTests(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expected: qbtypes.Statement{
				Query: "SELECT time AS `time`, id, trace_id, span_id, toUInt32OrZero(attributes['trace_flags']) AS `trace_flags`, severity_text, severity_number, attributes['scope.name'] AS `scope_name`, attributes['scope.version'] AS `scope_version`, body, attributes AS `attributes_string`, CAST(map(), 'Map(String, Float64)') AS `attributes_number`, CAST(map(), 'Map(String, Bool)') AS `attributes_bool`, map('service.name', toString(service)) AS `resources_string`, CAST(map(), 'Map(String, String)') AS `scope_string` FROM event.log WHERE match(LOWER(body), LOWER(?)) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query: "SELECT time, id, trace_id, span_id, trace_flags, severity_text, severity_number, scope_name, scope_version, body, attributes_string, attributes_number, attributes_bool, resources_string, scope_string FROM event.log WHERE (JSON_VALUE(body, '$.\"status\"') = ? AND JSON_EXISTS(body, '$.\"status\"')) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query: "SELECT time, id, trace_id, span_id, trace_flags, severity_text, severity_number, scope_name, scope_version, body, attributes_string, attributes_number, attributes_bool, resources_string, scope_string FROM event.log WHERE ((JSONExtract(JSON_QUERY(body, '$.\"user_names\"[*]'), 'Array(String)') = ?) AND JSON_EXISTS(body, '$.\"user_names\"[*]')) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query: "SELECT time, id, trace_id, span_id, trace_flags, severity_text, severity_number, scope_name, scope_version, body, attributes_string, attributes_number, attributes_bool, resources_string, scope_string FROM event.log WHERE has(JSONExtract(JSON_QUERY(body, '$.\"user_names\"[*]'), 'Array(String)'), ?) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
				Args:  []any{"john_doe", "1747947419000000000", uint64(1747945619), "1747983448000000000", uint64(1747983448), 10},
			},
			expectedErr: nil,
		},
...
func TestStmtBuilderBodyField(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expected: qbtypes.Statement{
				Query:    "SELECT time AS `time`, id, trace_id, span_id, toUInt32OrZero(attributes['trace_flags']) AS `trace_flags`, severity_text, severity_number, attributes['scope.name'] AS `scope_name`, attributes['scope.version'] AS `scope_version`, body_v2 as body, attributes AS `attributes_string`, CAST(map(), 'Map(String, Float64)') AS `attributes_number`, CAST(map(), 'Map(String, Bool)') AS `attributes_bool`, map('service.name', toString(service)) AS `resources_string`, CAST(map(), 'Map(String, String)') AS `scope_string` FROM event.log WHERE body_v2.message <> ? AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query: "SELECT time AS `time`, id, trace_id, span_id, toUInt32OrZero(attributes['trace_flags']) AS `trace_flags`, severity_text, severity_number, attributes['scope.name'] AS `scope_name`, attributes['scope.version'] AS `scope_version`, body, attributes AS `attributes_string`, CAST(map(), 'Map(String, Float64)') AS `attributes_number`, CAST(map(), 'Map(String, Bool)') AS `attributes_bool`, map('service.name', toString(service)) AS `resources_string`, CAST(map(), 'Map(String, String)') AS `scope_string` FROM event.log WHERE body <> ? AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
				Args:  []any{"", "1747947419000000000", uint64(1747945619), "1747983448000000000", uint64(1747983448), 10},
			},
			expectedErr: nil,
		},
		...
		},
		{
			...
			enableUseJSONBody: true,
			expected: qbtypes.Statement{
				Query:    "SELECT time AS `time`, id, trace_id, span_id, toUInt32OrZero(attributes['trace_flags']) AS `trace_flags`, severity_text, severity_number, attributes['scope.name'] AS `scope_name`, attributes['scope.version'] AS `scope_version`, body_v2 as body, attributes AS `attributes_string`, CAST(map(), 'Map(String, Float64)') AS `attributes_number`, CAST(map(), 'Map(String, Bool)') AS `attributes_bool`, map('service.name', toString(service)) AS `resources_string`, CAST(map(), 'Map(String, String)') AS `scope_string` FROM event.log WHERE body_v2.message ILIKE ? AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
		...
		},
		{
			...
			enableUseJSONBody: false,
			expected: qbtypes.Statement{
				Query: "SELECT time AS `time`, id, trace_id, span_id, toUInt32OrZero(attributes['trace_flags']) AS `trace_flags`, severity_text, severity_number, attributes['scope.name'] AS `scope_name`, attributes['scope.version'] AS `scope_version`, body, attributes AS `attributes_string`, CAST(map(), 'Map(String, Float64)') AS `attributes_number`, CAST(map(), 'Map(String, Bool)') AS `attributes_bool`, map('service.name', toString(service)) AS `resources_string`, CAST(map(), 'Map(String, String)') AS `scope_string` FROM event.log WHERE body ILIKE ? AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
				Args:  []any{"%error%", "1747947419000000000", uint64(1747945619), "1747983448000000000", uint64(1747983448), 10},
			},
			expectedErr: nil,
		},
...
func TestStmtBuilderBodyFullTextSearch(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			enableUseJSONBody: true,
			expected: qbtypes.Statement{
				Query:    "SELECT time AS `time`, id, trace_id, span_id, toUInt32OrZero(attributes['trace_flags']) AS `trace_flags`, severity_text, severity_number, attributes['scope.name'] AS `scope_name`, attributes['scope.version'] AS `scope_version`, body_v2 as body, attributes AS `attributes_string`, CAST(map(), 'Map(String, Float64)') AS `attributes_number`, CAST(map(), 'Map(String, Bool)') AS `attributes_bool`, map('service.name', toString(service)) AS `resources_string`, CAST(map(), 'Map(String, String)') AS `scope_string` FROM event.log WHERE match(LOWER(body_v2.message), LOWER(?)) AND time >= ? AND ts_bucket_start >= ? AND time < ? AND ts_bucket_start <= ? LIMIT ?",
				Args:     []any{"error", "1747947419000000000", uint64(1747945619), "1747983448000000000", uint64(1747983448), 10},
...
func TestSkipResourceFingerprintLogs(t *testing.T) {
	...
	t.Run("CTE attached when count below threshold", func(t *testing.T) {
		...
		mock.ExpectQueryRow(`SELECT count\(\) FROM \(SELECT fingerprint FROM event\.log_resource`).
