...
func TestFilterExprLogs(t *testing.T) {
	...
	testCases := []struct {
		...
		// Case-insensitive LIKE (ILIKE)
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?)",
			expectedArgs:          []any{"%error%", true},
			expectedErrorContains: "",
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (attributes_string['path'] ILIKE ? AND mapContains(attributes_string, 'path') = ?)",
			expectedArgs:          []any{"/api/%", true},
			expectedErrorContains: "",
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (attributes_string['email'] ILIKE ? AND mapContains(attributes_string, 'email') = ?)",
			expectedArgs:          []any{"%@EXAMPLE.com", true},
			expectedErrorContains: "",
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (attributes_string['filename'] ILIKE ? AND mapContains(attributes_string, 'filename') = ?)",
			expectedArgs:          []any{"%.PDF", true},
			expectedErrorContains: "",
		},
		...
		// NOT ILIKE
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE attributes_string['message'] NOT ILIKE ?",
			expectedArgs:          []any{"%error%"},
			expectedErrorContains: "",
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE attributes_string['path'] NOT ILIKE ?",
			expectedArgs:          []any{"/api/%"},
			expectedErrorContains: "",
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE attributes_string['email'] NOT ILIKE ?",
			expectedArgs:          []any{"%@EXAMPLE.com"},
			expectedErrorContains: "",
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE attributes_string['filename'] NOT ILIKE ?",
			expectedArgs:          []any{"%.PDF"},
			expectedErrorContains: "",
		},
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (attributes_string['level'] ILIKE ? AND mapContains(attributes_string, 'level') = ?)",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE attributes_string['level'] NOT ILIKE ?",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?) AND (attributes_string['level'] = ? AND mapContains(attributes_string, 'level') = ?))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((attributes_string['level'] = ? AND mapContains(attributes_string, 'level') = ?) OR (((attributes_string['level'] = ? AND mapContains(attributes_string, 'level') = ?) AND (attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?))))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((((attributes_string['level'] = ? AND mapContains(attributes_string, 'level') = ?) OR (attributes_string['level'] = ? AND mapContains(attributes_string, 'level') = ?))) AND NOT ((attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?)))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(attributes_number['count']) > ? AND mapContains(attributes_number, 'count') = ?) AND (attributes_string['level'] = ? AND mapContains(attributes_string, 'level') = ?) AND (attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (((attributes_string['level'] = ? AND mapContains(attributes_string, 'level') = ?) OR (((attributes_string['level'] = ? AND mapContains(attributes_string, 'level') = ?) AND (((attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?) OR (attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?)))))))",
		...
		// More common filter patterns
		{
			...
			shouldPass:    true,
			expectedQuery: "WHERE ((multiIf(resource.`service.name` IS NOT NULL, resource.`service.name`::String, mapContains(resources_string, 'service.name'), resources_string['service.name'], NULL) = ? AND multiIf(resource.`service.name` IS NOT NULL, resource.`service.name`::String, mapContains(resources_string, 'service.name'), resources_string['service.name'], NULL) IS NOT NULL) AND (((toFloat64(attributes_number['status']) >= ? AND mapContains(attributes_number, 'status') = ?) OR (toFloat64(attributes_number['duration']) > ? AND mapContains(attributes_number, 'duration') = ?))) AND NOT ((attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?)))",
		...
		`,
			shouldPass:    true,
			expectedQuery: "WHERE ((((((((toFloat64(attributes_number['status']) >= ? AND mapContains(attributes_number, 'status') = ?) AND (toFloat64(attributes_number['status']) < ? AND mapContains(attributes_number, 'status') = ?))) OR (((toFloat64(attributes_number['status']) >= ? AND mapContains(attributes_number, 'status') = ?) AND (toFloat64(attributes_number['status']) < ? AND mapContains(attributes_number, 'status') = ?) AND NOT ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?)))))) AND ((((multiIf(resource.`service.name` IS NOT NULL, resource.`service.name`::String, mapContains(resources_string, 'service.name'), resources_string['service.name'], NULL) = ? OR multiIf(resource.`service.name` IS NOT NULL, resource.`service.name`::String, mapContains(resources_string, 'service.name'), resources_string['service.name'], NULL) = ? OR multiIf(resource.`service.name` IS NOT NULL, resource.`service.name`::String, mapContains(resources_string, 'service.name'), resources_string['service.name'], NULL) = ?) AND multiIf(resource.`service.name` IS NOT NULL, resource.`service.name`::String, mapContains(resources_string, 'service.name'), resources_string['service.name'], NULL) IS NOT NULL) OR (((multiIf(resource.`service.type` IS NOT NULL, resource.`service.type`::String, mapContains(resources_string, 'service.type'), resources_string['service.type'], NULL) = ? AND multiIf(resource.`service.type` IS NOT NULL, resource.`service.type`::String, mapContains(resources_string, 'service.type'), resources_string['service.type'], NULL) IS NOT NULL) AND NOT ((multiIf(resource.`service.deprecated` IS NOT NULL, resource.`service.deprecated`::String, mapContains(resources_string, 'service.deprecated'), resources_string['service.deprecated'], NULL) = ? AND multiIf(resource.`service.deprecated` IS NOT NULL, resource.`service.deprecated`::String, mapContains(resources_string, 'service.deprecated'), resources_string['service.deprecated'], NULL) IS NOT NULL)))))))) AND (((((toFloat64(attributes_number['duration']) < ? AND mapContains(attributes_number, 'duration') = ?) OR ((toFloat64(attributes_number['duration']) BETWEEN ? AND ? AND mapContains(attributes_number, 'duration') = ?)))) AND ((multiIf(resource.`environment` IS NOT NULL, resource.`environment`::String, mapContains(resources_string, 'environment'), resources_string['environment'], NULL) <> ? OR (((multiIf(resource.`environment` IS NOT NULL, resource.`environment`::String, mapContains(resources_string, 'environment'), resources_string['environment'], NULL) = ? AND multiIf(resource.`environment` IS NOT NULL, resource.`environment`::String, mapContains(resources_string, 'environment'), resources_string['environment'], NULL) IS NOT NULL) AND (attributes_bool['is_automated_test'] = ? AND mapContains(attributes_bool, 'is_automated_test') = ?))))))) AND NOT ((((((attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?) OR (attributes_string['message'] ILIKE ? AND mapContains(attributes_string, 'message') = ?))) AND (attributes_string['severity'] = ? AND mapContains(attributes_string, 'severity') = ?)))))",
			expectedArgs: []any{
...
func TestFilterExprLogsConflictNegation(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (body NOT ILIKE ? AND attributes_string['body'] NOT ILIKE ?)",
		...
		},
		{
			...
			expectedQuery:         "WHERE (body NOT ILIKE ? AND attributes_string['body'] NOT LIKE ?)",
			expectedArgs:          []any{"done", "done"},
			expectedErrorContains: "",
		},
		{
