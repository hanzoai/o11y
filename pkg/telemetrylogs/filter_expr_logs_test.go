...
func TestFilterExprLogs(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (match(LOWER(body), LOWER(?)) AND (service = ? AND service <> ?))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (toBool(attributes['isEnabled'] = 'true') = ? AND mapContains(attributes, 'isEnabled') = ?)",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (toBool(attributes['is_valid'] = 'true') = ? AND mapContains(attributes, 'is_valid') = ?)",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((service = ? OR service = ? OR service = ?) AND service <> ?)",
		...
		// Explicit AND
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?) AND (service = ? AND service <> ?))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((service = ? AND service <> ?) OR (service = ? AND service <> ?))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE NOT ((service = ? AND service <> ?))",
			expectedArgs:          []any{"api"},
			expectedErrorContains: "",
		},
		{
		...
		// AND + OR combinations
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?) AND (((service = ? AND service <> ?) OR (service = ? AND service <> ?))))",
		...
		// AND + NOT combinations
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?) AND NOT ((service = ? AND service <> ?)))",
			expectedArgs:          []any{float64(200), true, "api"},
			expectedErrorContains: "",
		},
		{
		...
		// OR + NOT combinations
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (NOT ((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?)) OR NOT ((service = ? AND service <> ?)))",
			expectedArgs:          []any{float64(200), true, "api"},
			expectedErrorContains: "",
		},
		{
		...
		// AND + OR + NOT combinations
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?) AND (((service = ? AND service <> ?) OR NOT ((toFloat64(toFloat64OrNull(attributes['duration'])) > ? AND mapContains(attributes, 'duration') = ?)))))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (NOT ((((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?) AND (service = ? AND service <> ?)))) OR (toFloat64(toFloat64OrNull(attributes['count'])) > ? AND mapContains(attributes, 'count') = ?))",
		...
		// Mixed implicit and explicit AND
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?) AND (service = ? AND service <> ?) AND (toFloat64(toFloat64OrNull(attributes['duration'])) < ? AND mapContains(attributes, 'duration') = ?))",
			expectedArgs:          []any{float64(200), true, "api", float64(1000), true},
			expectedErrorContains: "",
		},
		{
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (((toFloat64(toFloat64OrNull(attributes['count'])) > ? AND mapContains(attributes, 'count') = ?) AND (((((toFloat64(toFloat64OrNull(attributes['duration'])) < ? AND mapContains(attributes, 'duration') = ?) AND (service = ? AND service <> ?))) OR (((toFloat64(toFloat64OrNull(attributes['duration'])) < ? AND mapContains(attributes, 'duration') = ?) AND (service = ? AND service <> ?)))))))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (toBool(attributes['isDisabled'] = 'true') = ? AND mapContains(attributes, 'isDisabled') = ?)",
			expectedArgs:          []any{false, true},
			expectedErrorContains: "",
		},
		{
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (toBool(attributes['is_invalid'] = 'true') = ? AND mapContains(attributes, 'is_invalid') = ?)",
			expectedArgs:          []any{false, true},
			expectedErrorContains: "",
		},
		...
		// Testing NOT > AND > OR precedence
		{
			...
			shouldPass:    true,
			expectedQuery: "WHERE (NOT ((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?)) AND (service = ? AND service <> ?))",
		...
		},
		{
			...
			shouldPass:    true,
			expectedQuery: "WHERE ((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?) OR ((service = ? AND service <> ?) AND (attributes['level'] = ? AND mapContains(attributes, 'level') = ?)))",
		...
		// Common filter patterns
		{
			...
			shouldPass:    true,
			expectedQuery: "WHERE (((toString(toFloat64OrNull(attributes['status'])) = ? OR toString(toFloat64OrNull(attributes['status'])) = ? OR toString(toFloat64OrNull(attributes['status'])) = ?) AND mapContains(attributes, 'status') = ?) AND NOT ((toBool(attributes['is_deleted'] = 'true') = ? AND mapContains(attributes, 'is_deleted') = ?)))",
		...
		// More common filter patterns
		{
			...
			shouldPass:    true,
			expectedQuery: "WHERE ((service = ? AND service <> ?) AND (((toFloat64(toFloat64OrNull(attributes['status'])) >= ? AND mapContains(attributes, 'status') = ?) OR (toFloat64(toFloat64OrNull(attributes['duration'])) > ? AND mapContains(attributes, 'duration') = ?))) AND NOT ((attributes['message'] ILIKE ? AND mapContains(attributes, 'message') = ?)))",
		...
		`,
			shouldPass:    true,
			expectedQuery: "WHERE ((((((((toFloat64(toFloat64OrNull(attributes['status'])) >= ? AND mapContains(attributes, 'status') = ?) AND (toFloat64(toFloat64OrNull(attributes['status'])) < ? AND mapContains(attributes, 'status') = ?))) OR (((toFloat64(toFloat64OrNull(attributes['status'])) >= ? AND mapContains(attributes, 'status') = ?) AND (toFloat64(toFloat64OrNull(attributes['status'])) < ? AND mapContains(attributes, 'status') = ?) AND NOT ((toFloat64(toFloat64OrNull(attributes['status'])) = ? AND mapContains(attributes, 'status') = ?)))))) AND ((((service = ? OR service = ? OR service = ?) AND service <> ?) OR (((attributes['service.type'] = ? AND mapContains(attributes, 'service.type') = ?) AND NOT ((attributes['service.deprecated'] = ? AND mapContains(attributes, 'service.deprecated') = ?)))))))) AND (((((toFloat64(toFloat64OrNull(attributes['duration'])) < ? AND mapContains(attributes, 'duration') = ?) OR ((toFloat64(toFloat64OrNull(attributes['duration'])) BETWEEN ? AND ? AND mapContains(attributes, 'duration') = ?)))) AND ((attributes['environment'] <> ? OR (((attributes['environment'] = ? AND mapContains(attributes, 'environment') = ?) AND (toBool(attributes['is_automated_test'] = 'true') = ? AND mapContains(attributes, 'is_automated_test') = ?))))))) AND NOT ((((((attributes['message'] ILIKE ? AND mapContains(attributes, 'message') = ?) OR (attributes['message'] ILIKE ? AND mapContains(attributes, 'message') = ?))) AND (attributes['severity'] = ? AND mapContains(attributes, 'severity') = ?)))))",
			expectedArgs: []any{
