...
func TestExistsConditionForWithEvolutions(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			operator:      qbtypes.FilterOperatorExists,
			value:         nil,
			// PIN MOVED with the envelope migration: the plane has ONE `service`
			// column, so an Exists on resource service.name is `service <> ''` on
			// every evolution — the old resources_string map no longer exists.
			expectedSQL:   "WHERE service <> ?",
			expectedArgs:  []any{""},
			expectedError: nil,
		},
		{
			...
			operator:      qbtypes.FilterOperatorExists,
			value:         nil,
			// PIN MOVED: same one-way answer — evolutions collapse onto the single
			// envelope `service` column; the JSON `resource` fallback died with it.
			expectedSQL:   "WHERE service <> ?",
			expectedArgs:  []any{""},
			expectedError: nil,
		},
...
func TestConditionFor(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			value:         float64(100),
			expectedSQL:   "(toFloat64(toFloat64OrNull(attributes['request.duration'])) > ? AND mapContains(attributes_number, 'request.duration') = ?)",
		...
		},
		{
			...
			value:         float64(1024),
			expectedSQL:   "(toFloat64(toFloat64OrNull(attributes['request.size'])) < ? AND mapContains(attributes_number, 'request.size') = ?)",
		...
		},
		{
			...
			operator:      qbtypes.FilterOperatorRegexp,
			value:         "frontend-.*",
			expectedSQL:   "WHERE (match(service, ?) AND `resource_string_service$$name_exists` = ?)",
