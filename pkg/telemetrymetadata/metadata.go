...
func NewTelemetryMetaStore(
	...
	metadataSettings := factory.NewScopedProviderSettings(settings, "github.com/hanzoai/o11y/pkg/telemetrymetadata")
...
func (t *telemetryMetaStore) tracesTblStatementToFieldKeys(ctx context.Context) ([]*telemetrytypes.TelemetryFieldKey, error) {
	...
	err := t.telemetrystore.Datastore().Select(ctx, &statements, query)
	if err != nil {
...
func (t *telemetryMetaStore) getLogsKeys(ctx context.Context, fieldKeySelectors []*telemetrytypes.FieldKeySelector) ([]*telemetrytypes.TelemetryFieldKey, bool, error) {
	...
	rows, err := t.telemetrystore.Datastore().Query(ctx, mainQuery, allArgs...)
	if err != nil {
...
func (k *telemetryMetaStore) fetchEvolutionEntryFromDatastore(ctx context.Context, selectors []*telemetrytypes.EvolutionSelector) ([]*telemetrytypes.EvolutionEntry, error) {
	...
	rows, err := k.telemetrystore.Datastore().Query(ctx, query, args...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
