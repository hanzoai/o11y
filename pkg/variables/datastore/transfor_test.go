...
WHERE service_name = $service_name
  ...
  AND op IN (SELECT op FROM event.operation WHERE service_name = $service_name AND kind = 'server')
  AND timestamp BETWEEN $start_time AND $end_time`,
			...
			expected: `SELECT trace_id, name, kind, status FROM event.span WHERE operation_name IN ($operation_names) AND duration >= $min_duration AND attributes['http.method'] = $http_method AND op IN (SELECT op FROM event.operation WHERE kind = 'server') AND timestamp BETWEEN $start_time AND $end_time;`,
		},
		{
...
WHERE service_name = $service_name
  ...
  AND timestamp BETWEEN $start_time AND $end_time`,
			...
			expected: `SELECT trace_id, name, kind, status FROM event.span WHERE attributes['http.method'] = $http_method AND op IN (SELECT op FROM event.operation WHERE kind = 'server') AND timestamp BETWEEN $start_time AND $end_time;`,
		},
		{
...
WHERE service_name = $service_name 
  ...
  AND timestamp BETWEEN $start_time AND $end_time`,
			...
			expected: `SELECT trace_id, name, kind, status FROM event.span WHERE op IN (SELECT op FROM event.operation WHERE kind = 'server') AND timestamp BETWEEN $start_time AND $end_time;`,
		},
		{
			name: "Example 3: Multiple __all__ selections and map lookups",
			sql: `SELECT trace_id, name, kind, status 
...
WHERE service_name = {{service_name}} 
  ...
  AND op IN (SELECT op FROM event.operation WHERE service_name = {{service_name}} AND kind = 'server')`,
			...
			expected: `SELECT trace_id, name, kind, status FROM event.span WHERE op IN (SELECT op FROM event.operation WHERE kind = 'server');`,
		},
	}
