...
// Dropped names the three signal databases HIP-0132 removed. A read that names
// one answers Code: 81 UNKNOWN_DATABASE.
//
// They are HERE because a test proving a query does not name them has to spell
// them, and a name spelled in a test file is a name in the tree — which is the
// exact thing plane_test.go refuses. One address for the fact, and the refusal
// keeps its teeth: a golden that PINS a dropped name still fails, because it
// pins a literal rather than ranging over this.
var Dropped = []string{"o11y_logs", "o11y_traces", "o11y_metrics"}
