...
func NewFactory(
	...
	return factory.NewProviderFactory(factory.MustNewName("o11y"), func(ctx context.Context, settings factory.ProviderSettings, config alertmanager.Config) (alertmanager.Alertmanager, error) {
...
func (provider *provider) Start(ctx context.Context) error {
	...
	ticker := time.NewTicker(provider.config.O11y.PollInterval)
	defer ticker.Stop()
	for {
		select {
		case <-provider.stopC:
