...
func newTestApp(t *testing.T) *zip.App {
	...
	if err := Use(app.Group(""), Deps{Logger: log, DataDir: t.TempDir(), Brand: "hanzo", Org: testOrg}); err != nil {
...
func TestMountRefusesWithoutATenantDecision(t *testing.T) {
	log := luxlog.NewNoOpLogger()
	app := zip.New(zip.Config{AppName: "metrics-test", Logger: log})
	if err := Use(app.Group(""), Deps{Logger: log, DataDir: t.TempDir(), Brand: "hanzo"}); err == nil {
