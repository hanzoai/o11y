...
// so the embedded o11y-engine route literals stay untouched (see rewriteExternalPath).
...
// single Hanzo-owned seam. It is done HERE, never by editing the embedded o11y-engine
// route literals: the o11y engine's whole frontend and backend speak /api/vN, and
// rewriting those literals is a fork diff a later upstream re-sync silently reverts (it
// happened once — see o11y/CLAUDE.md).
//
//	/v1/o11y/<resource>   → /api/<resource>   (canonical; resolves to a version-less
//	                                           alias — o11y engine, highest version wins —
...
//
// Because every rewritten path starts with /api/, the embedded o11y-engine StripPrefix
